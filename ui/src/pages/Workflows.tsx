...
import { Workflow, Calendar, CheckCircle, AlertCircle, Clock } from 'lucide-react';
import {
  startWorkflow,
  getWorkflowStatus,
  streamWorkflowWs,
...
export default function Workflows() {
      ...
      try {
        ...
        if (result.ok && result.jobs.length > 0) {
          ...
          if (morningJobs.length > 0) {
            const latestMorning = morningJobs[0];
  ...
  const handleStartWorkflow = async (workflowType: 'morning' | 'evening') => {
    try {
      ...
      // Show full placeholder state immediately
  ...
  const renderStep = (step: WorkflowStep, workflowType: 'morning' | 'evening') => {
    ...
    const isExpanded = expandedSteps[step.step_num] ?? true;
          ...
          <>
            ...
            {hasError && jobState.status.error && (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4">
                <div className="font-semibold text-red-600 dark:text-red-400 mb-1 flex items-center gap-2">
