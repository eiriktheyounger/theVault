...
- [project_calendar_icalpal_2026_04_21.md](project_calendar_icalpal_2026_04_21.md) — **icalPal fallback SHIPPED (2026-04-21)** + **dedup fix (2026-04-22)**: TCC-bypass calendar fetch via Ruby gem. `THEVAULT_CALENDAR_BACKEND` env override. Dedup by UID→tuple in `fetch_events_in_range` fixed 3–5× all-day event duplication; 04-01→04-21 DLYs backfilled.
- [project_morning_workflow_test_2026_04_22.md](project_morning_workflow_test_2026_04_22.md) — **Morning workflow end-to-end test (2026-04-22)**: Full clean run verified all 9 steps. Fixed 2 bugs: (1) `calendar_daily_injector.py` VAULT_ROOT path bug (same pattern as 04-20 weekly-summary fix); (2) `morning_workflow.py` `from System.Scripts.*` import failure → switched to sibling import. Final 04-22 DLY has complete structure (283 lines, 6 markers, 0 placeholders). Plaud: 2 groups written to Vault/Notes/, inbox drained except pending mp3.
